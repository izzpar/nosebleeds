        ...
        <div className="p-3.5">
          <div className="flex justify-between items-center mb-2">
            <div className="flex items-center gap-1.5">
              {g.ot && <span className="text-[9px] px-2 py-0.5 rounded-full bg-yellow-500/10 text-yellow-400 font-bold">OT</span>}
              <span className="text-[10px] font-bold text-zinc-500 tracking-wide uppercase">Wk {g.week} · {g.net} · {g.shortDate}</span>
            </div>
            <div className="flex gap-1">
              {g.diff <= 3 && g.isFinal && <span className="text-[9px] px-2 py-0.5 rounded-full bg-red-600/10 text-red-400 font-bold">CLOSE</span>}
              ...
              {[g.away, g.home].map((team, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-2.5 mb-1.5 last:mb-0">
                    ...
                    <span className={`flex-1 text-sm ${isWinner ? "font-bold text-white" : g.isPre ? "text-white" : "text-zinc-500"}`}>{team.name}</span>
                    <span className={`text-xl font-extrabold tabular-nums ${isWinner ? "text-white" : "text-zinc-700"}`}>{g.isPre ? "" : team.score}</span>
                  </div>
                );
              })}
            </div>
