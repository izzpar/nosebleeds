...
export default function MlbRecapPage({ params }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        // MLB ratings for this specific calendar date
        const rRes = await sbFetch(`ratings?game_date=eq.${date}&public=eq.true&rating=not.is.null&sport=eq.mlb&select=*`);
        ...
        if (gameIds.length > 0) {
          ...
          if (userIds.length > 0) {
            const pRes = await sbFetch(`profiles?user_id=in.(${userIds.join(",")})&select=user_id,handle,display_name,avatar_url,favorite_team_mlb`);
      ...
      } catch (e) { console.error("MLB recap load:", e); }
  ...
  useEffect(() => {
    ...
    async function loadPerformers() {
      ...
      if (gameIds.length === 0) { setTopHitters([]); setTopPitchers([]); return; }
      setPerfLoading(true);
      try {
        const hitters = [];
        const pitchers = [];
        ...
        for (const gid of gameIds) {
          const r = await fetch(`https://site.api.espn.com/apis/site/v2/sports/baseball/mlb/summary?event=${gid}`);
          if (!r.ok) continue;
          ...
          teams.forEach((team) => {
            ...
            (team.statistics || []).forEach((cat) => {
              ...
              (cat.athletes || []).forEach((a) => {
                ...
                if (type === "batting") {
                  const H = num(stats[idx("H")]), R = num(stats[idx("R")]);
                  const RBI = num(stats[idx("RBI")]), HR = num(stats[idx("HR")]);
                  const BB = num(stats[idx("BB")]), AB = num(stats[idx("AB")]);
                  ...
                  const IP = num(stats[idx("IP")]), K = num(stats[idx("K")]);
                  const ER = num(stats[idx("ER")]), BB = num(stats[idx("BB")]);
                  const H = num(stats[idx("H")]);
                  if (IP === 0) return;
        ...
        if (cancelled) return;
        // Dedupe by name (a player appears once per game), keep best score
        const dedupe = (arr) => {
          const best = {};
          arr.forEach(p => { if (!best[p.name] || p.score > best[p.name].score) best[p.name] = p; });
          return Object.values(best).sort((a, b) => b.score - a.score).slice(0, 5);
        };
        setTopHitters(dedupe(hitters));
        setTopPitchers(dedupe(pitchers));
  ...
  const gh = (gameId) => `/game/${gameId}?sport=mlb`;
    ...
    <div className="min-h-screen pb-24">
      ...
      <div className="sticky top-0 z-50 backdrop-blur-xl bg-[#09090b]/90 border-b border-zinc-800">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="text-sm font-bold text-white flex-1 text-center">⚾ Daily Recap</h1>
          <div className="w-12" />
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 pt-4">
        ...
        <div className="rounded-2xl overflow-hidden bg-gradient-to-br from-red-900/40 via-zinc-900 to-zinc-900 border border-zinc-800 p-5 mb-4 text-center">
          <div className="text-[10px] font-bold text-red-400 tracking-widest uppercase mb-1">⚾ MLB</div>
        ...
        {ratings.length === 0 ? (
          ...
          <>
            ...
            {/* Top Performers — by boxscore stats */}
            {(perfLoading || topHitters.length > 0 || topPitchers.length > 0) && (
              <div className="rounded-2xl bg-zinc-900 border border-zinc-800 p-4 mb-4">
                ...
                {perfLoading ? (
                  ...
                  <div className="space-y-4">
                    ...
                    {topHitters.length > 0 && (
                      <div>
                        <div className="text-[11px] font-extrabold tracking-widest uppercase text-green-400 mb-2">🏏 Top Hitters</div>
                        {topHitters.map((p, i) => (
                    ...
                    {topPitchers.length > 0 && (
                      <div>
                        <div className="text-[11px] font-extrabold tracking-widest uppercase text-blue-400 mb-2">⚾ Top Pitchers</div>
                        {topPitchers.map((p, i) => (
                          <Link key={p.name + i} href={`/player/${encodeURIComponent(p.name)}`} className="flex items-center gap-3 p-2.5 rounded-xl mb-1.5 bg-zinc-950 hover:bg-zinc-900 transition-colors">
                            <span className="text-base font-extrabold w-5 text-center" style={{ color: i === 0 ? "#fbbf24" : i === 1 ? "#a1a1aa" : i === 2 ? "#b45309" : "#52525b" }}>{i + 1}</span>
                            <div className="flex-1 min-w-0">
                  ...
                  <Link key={r.id} href={gh(r.game_id)} className="block">
                    <div className="p-3 rounded-xl mb-2 bg-zinc-950 hover:bg-zinc-900 transition-colors">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {r.profile?.favorite_team_mlb && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-red-600/15 text-red-300">⚾ {r.profile.favorite_team_mlb}</span>}
        ...
        <div className="flex gap-2 mt-6">
          <Link href={`/recap/mlb/${prevDate}`} className="flex-1 py-3 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-300 font-semibold text-sm text-center hover:bg-zinc-800">← Prev Day</Link>
          {canGoNext ? (
            <Link href={`/recap/mlb/${nextDate}`} className="flex-1 py-3 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-300 font-semibold text-sm text-center hover:bg-zinc-800">Next Day →</Link>
          ) : <div className="flex-1" />}
        </div>
      </div>
